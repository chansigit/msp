...
### Fixed
- `integrate`: tiny batches are excluded from the per-batch HVG vote (an IndexError on lineage subsets whose
  smallest batch had fewer cells than the HVG bin count).

### Changed
- Log lines carry a wall-clock stamp (bridge 0.2.4 logging format); `msp.log` tests updated accordingly.
