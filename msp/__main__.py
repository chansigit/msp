...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="msp", description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter
    )
...
def _integration_matches(out, args, harmony_kwargs):
    ...
    try:
        ...
        try:
            meta = _plain(integrated.uns.get("msp", {}))
        finally:
...
def _inspection_applied(out):
    """A completed inspection has mapped its verdicts onto integrated.h5ad."""
    if step_pending(out, "inspect") or not _done(
        out, "integrated.h5ad", "inspection_proposal.json", "figures/inspect_umap_action.png"
    ):
        return False
    try:
        import scanpy as sc
        ...
        try:
            return "_msp_action" in inspected.obs and "_msp_verdict" in inspected.obs
        finally:
...
def main(argv=None):
    ...
    if (
        args.force
        or step_pending(out, "integrate")
        or not _done(out, "integrated.h5ad")
        or not _integration_matches(out, args, harmony_kwargs)
    ):
        kw = dict(
    ...
    if args.inspect:
        if args.force or not _inspection_applied(out):
